...
const ChunkCard = (props) => {
  ...
  useEffect(() => {
    ...
    if (structureChunk) {
      // if (
      //   isFixedChunk(structureChunk) &&
      //   (!props.guideword || /^\d+$/.test(props.guideword))
      // ) {
      //   props.editGuidewordOnly(
      //     scUtils.improveGuideword(props.guideword, structureChunk)
      //   );
      // }
    ...
    if (props.formulaWasLoaded) {
      if (isFixedChunk(structureChunk)) {
        if (!props.guideword || /^\d+$/.test(props.guideword)) {
          structureChunk.guideword = {
            traitValue: scUtils.improveGuideword(
              props.guideword,
              structureChunk
            ),
          }; // to be transferred by bodge.
